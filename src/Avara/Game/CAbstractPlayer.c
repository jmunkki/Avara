...
void	CAbstractPlayer::SetSpecialColor(
	long	specialColor)
{
	CSmartPart	**thePart;
...
void	CAbstractPlayer::BeginScript()
{
	...
	yonBound = gApplication->ReadShortPref(kYonPrefTag, false)
				? SHORTYON : LONGYON;
...
void	CAbstractPlayer::PlaceHUDParts()
{
	...
	Matrix		m1, m2;
	Matrix		*mt;
	register	CBSPPart		*theSight;
	register	CAbstractActor	*theActor;
	register	CSmartPart		**thePartList;
				RayHitRecord	theHit;
				CWeapon			*weapon = NULL;
	...
	if(weaponIdent)	weapon = (CWeapon *)gCurrentGame->FindIdent(weaponIdent);
	...
	{	mt = &viewPortPart->itsTransform;
		...
		{
			OneMatrix(&m1);
			MTranslate(i ? gunOffset[0] : -gunOffset[0],
						gunOffset[1], gunOffset[2], &m1);
			...
			if(scoutView)	theHit.distance -= FIX3(200);
			else			theHit.distance += FIX3(100);
	
			if(theActor && (theActor->maskBits & kTargetBit))
			{	theSight = targetOns[i];
			
				itsGame->infoPanel->BrightBox(itsGame->frameNumber, theActor->GetPlayerPosition());
			}
			else
			{	theSight = targetOffs[i];
			}
	
			if(gunEnergy[i] < activeGunEnergy)		theSight->extraAmbient = 0;
			else if(gunEnergy[i] < fullGunEnergy)	theSight->extraAmbient = GUNREADYAMBIENT;
			else									theSight->extraAmbient = GUNFULLAMBIENT;
	
			theSight->isTransparent = false;
			theSight->Reset();
			if(i)	PreFlipZ(theSight);

			if(debug2Flag)
			{	theSight->ApplyMatrix(&m2);
				TranslatePart(theSight, FMul(theHit.distance, theHit.direction[0]),
...
void	CAbstractPlayer::ReturnWeapon(
	short	theKind)
{
	switch(theKind)
	{	case kweSmart:
...
void	CAbstractPlayer::ArmSmartMissile()
{
	...
	didDetach = SignalAttachments(kBallReleaseSignal, (long)this);
...
Boolean	newWayFlag = false;

void	CAbstractPlayer::KeyboardControl(
	FunctionTable	*ft)
{
	if(ft)
	{
		...
		if(viewYaw < -maxYaw)		viewYaw = -maxYaw;
		if(viewYaw > maxYaw)		viewYaw = maxYaw;
		if(viewPitch < minPitch)	viewPitch = minPitch;
		if(viewPitch > maxPitch)	viewPitch = maxPitch;
		...
		{	modAccel = FDivNZ(baseMass, GetTotalMass());
			...
			if(TESTFUNC(kfuForward, ft->held))	motionFlags |= 1+2;
			if(TESTFUNC(kfuReverse, ft->held))	motionFlags |= 4+8;
			if(TESTFUNC(kfuLeft, ft->held))		motionFlags |= 2+4;
			if(TESTFUNC(kfuRight, ft->held))	motionFlags |= 1+8;

			if(motionFlags & 1)			motors[0] += modAccel;
			if(motionFlags & 2)			motors[1] += modAccel;
			if(motionFlags & 4)			motors[0] -= modAccel;
			if(motionFlags & 8)			motors[1] -= modAccel;

			if(TESTFUNC(kfuBoostEnergy, ft->down) &&
				boostsRemaining && (boostEndFrame < itsGame->frameNumber))
			...
			if(mouseShootTime-- <= 0)
			{	if(ft->buttonStatus & kbuIsDown) 		fireGun = true;
				if(TESTFUNC(kfuFireWeapon, ft->held))	fireGun = true;
			}
			if(ft->buttonStatus & kbuWentDown)			fireGun = true;
			if(TESTFUNC(kfuFireWeapon, ft->down))		fireGun = true;
		...
		{	if(limboCount > 0 || lives == 0)
			{	if(didSelfDestruct || winFrame >= 0 || lives == 0) 
				{	isOut = true;
					...
					itsGame->infoPanel->DrawUserInfoPart(itsManager->slot, kipDrawColorBox);
			...
			else
			{
				...
				{	itsGame->infoPanel->MessageLine(kmSelfDestruct, centerAlign);
					if(lives > 1)
						itsGame->infoPanel->MessageLine(kmSelfDestruct2, centerAlign);
				}
		...
		{	Boolean		doRelease = false;
			...
			{	if(TESTFUNC(kfuForward, ft->down))	scoutCommand = kScoutLead;
				if(TESTFUNC(kfuReverse, ft->down))	scoutCommand = kScoutFollow;
				if(TESTFUNC(kfuLeft, ft->down))		scoutCommand = kScoutLeft;
				if(TESTFUNC(kfuRight, ft->down))	scoutCommand = kScoutRight;
				if(TESTFUNC(kfuAimForward, ft->down))	scoutCommand = kScoutUp;
			}
		...
		if(TESTFUNC(kfuDebug1, ft->down))		debugView = !debugView;
		if(TESTFUNC(kfuDebug2, ft->down))		debug2Flag = !debug2Flag;
		if(TESTFUNC(kfuZoomIn, ft->held))		fieldOfView -= FOVSTEP;
		if(TESTFUNC(kfuZoomOut, ft->held))		fieldOfView += FOVSTEP;
...
#define	MAXSIDELOOK	0x8000L
		...
		if(TESTFUNC(kfuLookRight, ft->held))
		{	lookDirection += LOOKSTEP;
			if(lookDirection > MAXSIDELOOK)
				lookDirection = MAXSIDELOOK;
		}
		...
		{	short	mode;
			...
			if(mode)	yonBound = SHORTYON;
			else		yonBound = LONGYON; 

			if(itsManager->isLocalPlayer)
			{	gApplication->WriteShortPref(kYonPrefTag, mode);
				itsGame->infoPanel->MessageLine(kmLongView - mode, centerAlign);
			}
		}

		if(fieldOfView < MINFOV) fieldOfView = MINFOV;
		if(fieldOfView > MAXFOV) fieldOfView = MAXFOV;

		if(fireGun) mouseShootTime = MOUSESHOOTDELAY;
...
void	CAbstractPlayer::GetSpeedEstimate(
	Fixed	*theSpeed)
{
	theSpeed[0] = speed[0];
	theSpeed[1] = speed[1];
...
void	CAbstractPlayer::PlayerAction()
{
	...
	{	itsGame->playersStanding++;
		if((itsGame->frameNumber & 2047) == 2047 &&
			itsGame->playersStanding == 1 && itsManager->isLocalPlayer)
...
void	CAbstractPlayer::GunActions()
{
	...
	{	CWeapon		*weapon = NULL;
		...
		if(weaponIdent)	weapon = (CWeapon *)itsGame->FindIdent(weaponIdent);
		...
		{	i = gunEnergy[0] < gunEnergy[1];
			...
			{	Vector	missileSpeed;
				...
				theMissile = (CPlayerMissile *)
								itsGame->itsDepot->LaunchMissile(
...
void	CAbstractPlayer::PostMortemBlast(
	short	scoreTeam,
	short	scoreColor,
	Boolean	doDispose)
{
	...
	inherited::PostMortemBlast(scoreTeam, scoreColor, false);
	...
	{	itsGame->infoPanel->MessageLine(kmGameOver, centerAlign);
	}

	itsGame->infoPanel->DrawUserInfoPart(itsManager->position, kipDrawColorBox);
}

void	CAbstractPlayer::GoLimbo(
	long	limboDelay)
{
	CSmartPart	**thePart;
	...
	CIncarnator	*newSpot)
{
	...
	if(itsManager)
		itsManager->resetMousePeriod = 0;

	if(newSpot == NULL)
	{	placeList = itsGame->incarnatorList;

		while(placeList)
		{	if( placeList->enabled &&
				(placeList->colorMask & teamMask) &&
				(placeList->useCount < bestCount))
...
#ifdef SPECIAL_ACCEL
void	CAbstractPlayer::Accelerate(
	Fixed	*direction)
{
	direction[1] >>= 2;
	inherited::Accelerate(direction);
}
#endif

Boolean	CAbstractPlayer::TryTransport(
	Fixed	*where,
	short	soundId,
	Fixed	volume,
	short	options)
{
	...
	oldLoc[0] = location[0];	location[0] = where[0];
	oldLoc[1] = location[1];	location[1] = where[1];
	oldLoc[2] = location[2];	location[2] = where[2];
...
void	CAbstractPlayer::Win(
	long			winScore,
	CAbstractActor	*teleport)
{
	short		count = 16;
	...
	if(itsManager->isLocalPlayer)
	{	itsGame->infoPanel->MessageLine(kmWin, centerAlign);
	}
...
void	CAbstractPlayer::ReceiveConfig(
...
{
	return    (((long)boostsRemaining)<<18)
			+ (((long)grenadeCount)<<16)
			+ (((long)missileCount)<<16)
			+ inherited::GetTotalMass();
}
...
void	CAbstractPlayer::TakeGoody(
	GoodyRecord		*gr)
{
	grenadeCount += gr->grenades;
	if(grenadeCount > grenadeLimit)	grenadeCount = grenadeLimit;

	missileCount += gr->missiles;
	if(missileCount > missileLimit)	missileCount = missileLimit;

	boostsRemaining += gr->boosters;
	if(boostsRemaining > boosterLimit)	boostsRemaining = boosterLimit;
	lives += gr->lives;
	if(gr->lives)
	{	itsGame->infoPanel->DrawUserInfoPart(itsManager->position, kipDrawColorBox);
	}
...
short	CAbstractPlayer::GetBallSnapPoint(
	long		theGroup,
	Fixed		*ballLocation,
	Fixed		*snapDest,
	Fixed		*delta,
	CSmartPart	**hostPart)
{
	...
	{	Fixed	globDelta[3];
		...
		*hostPart = viewPortPart;
		VectorMatrix34Product(1, (Vector *)delta, (Vector *)snapDest, &viewPortPart->itsTransform);
