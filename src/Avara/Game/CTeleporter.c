...
void	CTeleporter::FrameAction()
{
	...
	{	CAbstractPlayer	*thePlayer;
		...
		if(noPullTimer)	noPullTimer--;
		
		while(thePlayer)
		{	if( (thePlayer->teamMask & watchTeams) &&
				(thePlayer->searchCount != searchCount) &&
				!thePlayer->isInLimbo &&
				thePlayer->itsGame->scores[thePlayer->itsManager->slot] >= hitScore)
			{	Vector	delta;
				Fixed	distance;
				...
				else if(noPullTimer == 0 &&
						distance < TELEPORTAREA)
	...
	CAbstractPlayer	*thePlayer)
{
	...
	{	if(theActor->maskBits & kTeleportBit)
		{	if(	theActor->transportGroup == destGroup &&
				theActor->useCount < maxUse &&
				theActor != this)
...
Boolean	CTeleporter::ReceivePlayer(
	CAbstractPlayer *thePlayer)
{
	Boolean	didMove;
