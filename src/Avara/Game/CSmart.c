...
void	CSmart::ShowTarget()
{
	...
	itsGame->infoPanel->BrightBox(itsGame->frameNumber, inSight);
	...
	{	v = partList[0]->itsTransform[2];
		...
		TranslatePart(targ,	FMul(v[0], dist),
							FMul(v[1], dist),
							FMul(v[2], dist));
		targ->MoveDone();
...
#define MOD_THRUST
	...
	if(delta < -angleStep)		yaw -= angleStep;
	else if(delta > angleStep)	yaw += angleStep;
	else						yaw = goodYaw;
	yaw = (short)yaw;

	delta = goodPitch - pitch;
	if(delta < -angleStep)		pitch -= angleStep;
	else if(delta > angleStep)	pitch += angleStep;
...
void	CSmart::FrameAction()
{
	...
	{	Vector		accel;
		...
		speedDotAccel = FMul(accel[0], speed[0]) +
						FMul(accel[1], speed[1]) +
						FMul(accel[2], speed[2]);
