...
void	CWalkerActor::LoadParts()
{
	...
	viewPortPart = partList[0];	
	proximityRadius = viewPortPart->header.enclosureRadius<<2;
}
...
#define	DIFFERENTLEGS(a,b)	((a.highAngle != b.highAngle) || (a.lowAngle != b.lowAngle))
				...
				case undoLegs:
#ifdef TESTIT
					if(	DIFFERENTLEGS(legs[0],legUndo[0]) ||
						DIFFERENTLEGS(legs[1],legUndo[1]) ||
						legPhase != phaseUndo)
#endif
					{	legs[0] = legUndo[0];
						legs[1] = legUndo[1];
...
void	CWalkerActor::DoStandingTouches()
{
	...
	{	touchActor = itsGame->FindIdent(legs[1].touchIdent);
		if(touchActor)
		{
			...
			touchActor->StandingOn(this, legs[1].where,
...
void	CWalkerActor::DoLegTouches()
{
	...
	{	touchActor[0] = itsGame->FindIdent(legs[0].touchIdent);
		if(touchActor[0])
			touchActor[0]->StandOn(this, legs[0].where,
									true,
									(legUndo[0].touchIdent != legs[0].touchIdent) &&
									(legUndo[1].touchIdent != legs[0].touchIdent));
	}
	else
	{	touchActor[0] = NULL;
	}
	...
	{	touchActor[1] = itsGame->FindIdent(legs[1].touchIdent);
		if(touchActor[1])
			touchActor[1]->StandOn(this, legs[1].where,
									legs[0].touchIdent != legs[1].touchIdent,
									(legUndo[0].touchIdent != legs[1].touchIdent) &&
									(legUndo[1].touchIdent != legs[1].touchIdent));
	}
	else
	{	touchActor[1] = NULL;
	}
...
void	CWalkerActor::MoveLegs()
{
	...
	{	Fixed	moveRadius;
		...
		if(moveRadius < 0) moveRadius = -moveRadius;
...
void	CWalkerActor::TractionControl()
{
	...
	if(motorFriction < 0) motorFriction = - motorFriction;
	...
	if(bounceTarget > 0) bounceTarget = -bounceTarget;
	...
	inherited::TractionControl();
}

void	CWalkerActor::MotionControl()
{
	inherited::MotionControl();
	
	MoveLegs();
}

void	CWalkerActor::KeyboardControl(
	FunctionTable	*ft)
{
	...
	{	if(TESTFUNC(kfuVerticalMotion, ft->held))
		{	viewPitch = oldPitch;
			...
			if(stance > MAXHEADHEIGHT) stance = MAXHEADHEIGHT;
			else if(stance < MINHEADHEIGHT) stance = MINHEADHEIGHT;
...
void	CWalkerActor::ReceiveConfig(
	PlayerConfigRecord	*config)
{
	...
	{	short				hullRes;
		...
		if(hullRes < 0 || hullRes > 2) hullRes = 1;
	...
	inherited::ReceiveConfig(config);
}