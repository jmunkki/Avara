...
void	CSwitchActor::FrameAction()
{
	...
	{	while(enabled && toggleCount--)
		{	theState = !theState;
			if(theState)	itsGame->FlagMessage(onMessage);
			else			itsGame->FlagMessage(offMessage);
