/*
    ...
    Modified: Saturday, September 14, 1996, 0:21
*/
...
CSmartPart *	CDoorActor::CollisionTest()
{
	...
	{	theActor = thePart->theOwner;
		if(!(theActor->maskBits & kDoorIgnoreBit) &&
	...
	{	BlastHitRecord	theBlast;
		...
		{	anActor = (CAbstractActor *)thePart->theOwner;
			if(	anActor &&
				...
				!(anActor->maskBits & kDoorIgnoreBit) &&
				partList[0]->CollisionTest(thePart))
...
void	CDoorActor::PlaceParts()
{
	...
	if(stat)
	{	r = FMul(twists[0], stat);	if(r)	theDoor->RotateX(r);
		r = FMul(twists[1], stat);	if(r)	theDoor->RotateY(r);
		r = FMul(twists[2], stat);	if(r)	theDoor->RotateZ(r);
...
CAbstractActor *CDoorActor::EndScript()
{
	...
	{	CBSPWorld	*theWorld;
		...
		if(openSpeed <= 0 || openSpeed > kDoorOpen) openSpeed = kDoorSpeed;
		
		closeSpeed = ReadFixedVar(iCloseSpeed)/100;
		if(closeSpeed <= 0 || closeSpeed > kDoorOpen) closeSpeed = kDoorSpeed;
...
void	CDoorActor::DoorSound()
{
	...
	if(itsSoundLink)	gHub->ReleaseLinkAndKillSounds(itsSoundLink);
...
//			isActive |= isActive;
	...
	if(action != kDoorStopped)
	{
		...
		{	doorStatus -= closeSpeed;
			if(doorStatus < kDoorClosed) doorStatus = kDoorClosed;
		}
		
		UnlinkLocation();
		PlaceParts();

		BuildPartProximityList(partList[0]->sphereGlobCenter, partList[0]->header.enclosureRadius, kSolidBit);
		if(CollisionTest())
		{	TouchDamage();
...
{
	inherited::StandingOn(who, where, firstLeg);
