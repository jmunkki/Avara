	...
	CParasite::EndScript()
{
	...
	{	gHub->PreLoadSample(clampSound = ReadLongVar(iSound));
		...
		location[1] -= partList[0]->header.minBounds.x;
...
void	CParasite::FrameAction()
{
	...
	{	short	delta;
		...
		{	
			...
			if(delta < -kAngleStep)		heading -= kAngleStep;
			else if(delta > kAngleStep)	heading += kAngleStep;
			else						heading = goodHeading;
			heading = (short)heading;
	
			delta = goodPitch - pitch;
			if(delta < -kAngleStep)		pitch -= kAngleStep;
			else if(delta > kAngleStep)	pitch += kAngleStep;
			else						pitch = goodPitch;
		}
		else
		{	course[0] >>= 1;
			if(course[0] == -1) course[0] = 0;
			course[2] >>= 1;	course[2]++;
			if(course[2] == -1) course[2] = 0;
			if(location[1] > -partList[0]->header.minBounds.x)
				course[1] = FIX3(-10);
			else
				course[1] = 0;

			delta = 0x4000 - pitch;
			if(delta < -kSlowAngleStep)		pitch -= kSlowAngleStep;
			else if(delta > kSlowAngleStep)	pitch += kSlowAngleStep;
			else							pitch = 0x4000;
		}


		if(pitch != oldPitch || oldHeading != heading ||
		...
		{	location[0] += speed[0];
			...
			PlaceParts();
			BuildPartProximityList(location, partList[0]->header.enclosureRadius, kSolidBit);
