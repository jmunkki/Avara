...
void	CGuardActor::FrameAction()
{
	...
	{	CSmartPart	*target;
		...
		{	Vector	delta;
			...
			if(angleChange < -rotSpeed)		heading -= rotSpeed;
			else if(angleChange > rotSpeed)	heading += rotSpeed;
			else						heading = goodHeading;
			heading = (short)heading;
	
			angleChange = goodPitch - pitch;
			if(angleChange < -rotSpeed)		pitch -= rotSpeed;
			else if(angleChange > rotSpeed)	pitch += rotSpeed;
		...
		if(isTracking || fireActivator.triggerCount)
		{
			...
			{	theActor = theHit.closestHit->theOwner;
				...
				{	CAbstractMissile	*theMissile;
					...
					theHit.playerId = -1;
					theMissile = itsGame->itsDepot->LaunchMissile(kmiFlat,
											&partList[0]->itsTransform, 
											&theHit,
											this,
											shotPower, NULL);
					if(theMissile)
