...
void	CDoor2Actor::PlaceParts()
{
	...
	r = endTwist[0] + FMul(deltaTwist[0], stat);	if(r)	theDoor->RotateX(r);
	r = endTwist[1] + FMul(deltaTwist[1], stat);	if(r)	theDoor->RotateY(r);
	r = endTwist[2] + FMul(deltaTwist[2], stat);	if(r)	theDoor->RotateZ(r);
...
{
	inherited::ProgramDoorVariables();
