...
void	CRealMovers::GetSpeedEstimate(
	Fixed	*theSpeed)
{
	*theSpeed++ = speed[0];
	*theSpeed++ = speed[1];
...
void	CRealMovers::FindBestMovement(
	CSmartPart	*objHit)
{
	Vector	newSpeed;
	Vector	deltaSpeed;
	...
	{	
		...
		{	
			...
			if(newSpeed[0] > MINSPEED || newSpeed[0] < -MINSPEED ||
				newSpeed[1] > MINSPEED || newSpeed[1] < -MINSPEED ||
				newSpeed[2] > MINSPEED || newSpeed[2] < -MINSPEED)
			{	
