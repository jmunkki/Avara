	...
	CUfo::EndScript()
{
	...
	{	short	shapeRes;
		...
		if(location[1] < -partList[0]->header.minBounds.y)
			location[1] = -partList[0]->header.minBounds.y;
		...
		if(verticalMin < -partList[0]->header.minBounds.y)
			verticalMin = -partList[0]->header.minBounds.y;
...
void	CUfo::Navigate()
{
	...
	if(alpha < -kAngleTolerance)		turnRate -= kAngleStep;
	else if(alpha > kAngleTolerance)	turnRate += kAngleStep;

	heading += turnRate;
	turnRate = FMul(turnRate, kUfoTurnFriction);

	alpha = goodPitch - pitch;
	if(alpha < -kAngleStep)		pitch -= kAngleStep;
	else if(alpha > kAngleStep)	pitch += kAngleStep;
	else						pitch = goodPitch;
	...
	Boolean			doAttack)
{
	...
	{	
		if(homeRange >= 0)
		{	placeScore = homeRange - FDistanceEstimate(		homeBase[0] - position[0],
											homeBase[1] - position[1],
											homeBase[2] - position[2]);
			if(placeScore > 0)
			{	placeScore = 0;
			}
			else
		...
		{	placeScore = FDistanceEstimate(	homeBase[0] - position[0],
											homeBase[1] - position[1],
											homeBase[2] - position[2]);
...
void	CUfo::ReTarget()
{
	...
	{	randRange = FMul(motionRange - (motionRange>>2), FRandom()) + (motionRange>>2);
		...
		if(dir < 0)	dir = heading - FMul(dir, dir);
		else		dir = heading + FMul(dir, dir);
