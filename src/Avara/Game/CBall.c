...
void	CBall::ChangeOwnership(
	short		ownerId,
	short		ownerTeamColor)
{
...
void	CBall::MagnetAction()
{
	...
	{	short		snapCode;
		...
		if(	(		theActor->ident != oldHost
				|| looseFrame < thisFrame)
			&&	!(	theActor->maskBits & kPlayerBit
				&&	holdShieldLimit < shields
				&&	theActor->teamColor != teamColor))
		{	snapCode = theActor->GetBallSnapPoint(group, location, snapTo, localSnap, &newHost);
			...
			{	Matrix			savedMatrix;
				...
				{	playerAttach = (hostActor->maskBits & kPlayerBit) != 0;
					...
					hostActor->ReceiveSignal(kBallAttached, (long)this);
...
void	CBall::FrameAction()
{
	...
	{	CAbstractActor	*theOwner;
		...
		{	theOwner = itsGame->FindIdent(hostIdent);
			...
			{	itsGame->scoreReason = ksiHoldBall;
				itsGame->Score(theOwner->teamColor, theOwner->GetActorScoringId(),
								carryScoreAccumulator >> 16, 0, 0, -1);
			}
	...
	if(status && hostPart == NULL)
	{
		...
		if(location[1] > partList[0]->header.minBounds.y)
		...
		if(speed[1] < 0 &&
			speed[1] + location[1] < -partList[0]->header.minBounds.y)
		{	speed[1] = -partList[0]->header.minBounds.y - location[1];
		}

		if(speed[0] > MINSPEED || speed[0] < -MINSPEED ||
			speed[1] > MINSPEED || speed[1] < -MINSPEED ||
		...
		{	VECTORCOPY(oldLocation, location);
			...
			location[2] += speed[2];
			OffsetParts(speed);
			BuildPartProximityList(location, partList[0]->bigRadius + 
									FDistanceOverEstimate(speed[0], speed[1], speed[2]),
									kSolidBit);
		...
		if(	shields >= shootShields &&
			shotPower && itsGame->frameNumber >= nextShotFrame)
		{	Boolean	didShoot;
