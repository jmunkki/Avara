...
void	CZombieActor::RememberExperience(
	Fixed	*place,
	Fixed	weight)
{
	Fixed	*smarts;
	...
	if(nextSmart >= SMARTMEMSIZE)	nextSmart = 0;
...
void	CZombieActor::MoveForward()
{
	...
	if(delta < -zombieTurn)		heading -= zombieTurn;
	else if(delta > zombieTurn)	heading += zombieTurn;
...
void	CZombieActor::FrameAction()
{
	...
	{	Fixed			newHeading;
		...
		if(watchFlip > 0)	watchFlip = - watchFlip - 100;
		else				watchFlip = - watchFlip + 100;
