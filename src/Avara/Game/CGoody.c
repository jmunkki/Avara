		...
		CGoody::EndScript()
{
	...
	{	short	shapeId;
		...
		if(partList[1])	partList[1]->isTransparent = enabled;
...
void	CGoody::FrameAction()
{
	...
	{	BuildPartProximityList(partList[0]->sphereGlobCenter, partList[0]->header.enclosureRadius,
		...
		{	if(	thePart->CollisionTest(partList[0]))
			{	CAbstractPlayer	*theActor;
				...
				itsGame->Score(theActor->teamColor, theActor->GetActorScoringId(),
								hitScore, 0, teamColor, GetActorScoringId());
				break;
			}
		}
	...
	if(enabled)	sleepTimer = frequency;
}
