...
void	InitSymbols()
{
	...
	symTable->AddDictEntry("\penum",-1);
	symTable->AddDictEntry("\punique",-1);
	symTable->AddDictEntry("\pend",-1);
	symTable->AddDictEntry("\padjust",-1);
	lastKeyword = symTable->AddDictEntry("\pobject",-1);
	...
	LexSymbol	*theSymbol)
{
	...
	while(theChar && (theChar <= 32 || theChar == 'Ê'))	//	Option space!!
	{	theChar = *(++parserVar.input);
	}
...
void	ParseTerm()
{
	...
	while(parserVar.lookahead.kind == kLexMultiply ||
...
void	ParseExpr()
{
	...
	while(	parserVar.lookahead.kind == kLexPlus ||
...
void	ParseMinMax()
{
	...
	while(	parserVar.lookahead.kind == kLexMin ||
...
void	ParseCompare()
{
	...
	while(	parserVar.lookahead.kind == kLexGt ||
			parserVar.lookahead.kind == kLexEq ||
			parserVar.lookahead.kind == kLexLt ||
			parserVar.lookahead.kind == kLexLtEq ||
		...
		{	while(theProgram->kind != kAssignment)
			{	switch(theProgram->kind)
				{	case kLexConstant:
					...
					case kLexGtEq:		stackP[-1] = stackP[-1] >= stackP[0];	stackP--;	break;
					case kLexLtEq:		stackP[-1] = stackP[-1] <= stackP[0];	stackP--;	break;
					...
					case kLexFun1:
						...
						{	
							case kSinKey:	*stackP = sin(*stackP * (3.14159265359 / 180));	break;
							case kCosKey:	*stackP = cos(*stackP * (3.14159265359 / 180));	break;
...
void	DeallocParser()
{
	if(theScript)	DisposeHandle(theScript);
	if(stackP)		DisposePtr((Ptr)stackP);

	if(symTable)		symTable->Dispose();
	if(variableBase)	variableBase->Dispose();
	if(programBase)		programBase->Dispose();
	...
	short	index)
{
	...
	return (((theColor / 1000000) << 16) |
			(((theColor / 1000) % 1000) << 8) |
			(theColor % 1000));
}
	...
	long	addValue)
{
	...
	index += firstVariable;
	ref += firstVariable;
	steps[0].kind = kLexVariable;	steps[0].value.token = ref;
	steps[1].kind = kLexConstant;	steps[1].value.floating = addValue;
	steps[2].kind = kLexPlus;
	steps[3].kind = kAssignment;	steps[3].value.token = index;

	programBase->WriteEntry(index, sizeof(LexSymbol)*4, (Ptr)steps);
}

void	ProgramOffsetMultiply(
	short	index,
	short	ref,
	long	multValue)
{
	...
	ref += firstVariable;
	steps[0].kind = kLexVariable;	steps[0].value.token = ref;
	steps[1].kind = kLexConstant;	steps[1].value.floating = multValue;
