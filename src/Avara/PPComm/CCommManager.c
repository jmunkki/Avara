...
#define	PACKETDEBUGno
#define	DEBUGPACKETSTATUS	0
#ifdef	PACKETDEBUG
static	long	inOutCount;
#endif
...
**	for buffers.
*/
void	CCommManager::ICommManager(
	short	packetSpace)
{
	OSErr			iErr;
	long			attr;
...
OSErr	CCommManager::AllocatePacketBuffers(
	short	packetSpace)
{
	OSErr			theErr;
	Ptr				mem;
	...
	Ptr			dataPtr)
{
	...
	{	thePacket = GetPacket();
		...
		{	thePacket->flags = 0;
			...
			if(dataLen > PACKETDATABUFFERSIZE) dataLen = PACKETDATABUFFERSIZE;
...
**	send the packet.
*/
OSErr	CCommManager::SendUrgentPacket(
	short		distribution,
	char		command,
	char		p1,
	short		p2,
...
*/
void	CCommManager::WriteAndSignPacket(
	PacketInfo	*thePacket)
{
	thePacket->sender = 0;
...
*/
void	CCommManager::AddReceiver(
	ReceiverRecord	*theReceiver,
	Boolean			delayed)
{
...
*/
void	CCommManager::RemoveReceiver(
	ReceiverRecord	*theReceiver,
	Boolean			delayed)
{
...
PacketInfo	*CCommManager::GetPacket()
{
	...
	if(thePacket == NULL)
	{	
		DebugStr("\pOut of packet buffers!");
	}
	else
#endif
	{	freeCount--;
	...
	if(thePacket) 
	{
#if DEBUGPACKETSTATUS
		thePacket->debugStatus = 0;
#endif
		inOutCount++;
		if(inOutCount > 100)
		{	DebugStr("\P100 PacketBuffers in use.;g");
		}
	}
#endif
...
**	another one.
*/
PacketInfo	*CCommManager::DuplicatePacket(
	PacketInfo	*original)
{
...
*/
void	CCommManager::ReleasePacket(
	PacketInfo	*thePacket)
{
	if(thePacket)
	{	freeCount++;
		...
		inOutCount--;
#if DEBUGPACKETSTATUS
		if(thePacket->debugStatus)
			thePacket->debugStatus = 0;
		else
			thePacket->debugStatus = 1;
#endif
#endif
...
void	CCommManager::ProcessQueue()
{
	...
	{	thePacket = (PacketInfo *)inQ.qHead;
		...
		{	short	iErr;
			...
			{	DebugStr("\pDequeue failed in ProcessQueue!!!");
				thePacket = NULL;
			}
#endif
		}
...
**	is to do nothing.
*/
void	CCommManager::DisconnectSlot(
	short	slotId)
{
}

void	CCommManager::OptionCommand(
	long	theCommand)
{

}

short	CCommManager::GetStatusInfo(
	short	slot,
	Handle	leftColumn,
	Handle	rightColumn)
{
...
long	CCommManager::GetMaxRoundTrip(
	short	distribution)
{
	return	0;	//	Local net.
}