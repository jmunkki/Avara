...
OSErr	CTracker::PrepareToSend()
{
	...
	{	sendPB.udpStream = stream;
		...
		sendPB.csParam.send.wdsPtr = (void *)writeDataTable;
		...
		{	char			theTag;
			...
			{	char	index;
				...
				{	case kisPlayerIPPort:
						...
						break;
					case kisPlayerLives:
						if(*p.uw >= 0)	NumToString(*p.uw, tempStr);
	...
	{	case updateEvt:
		...
		case autoKey:
			{	char	theChar;
				...
				if(didHandle && theEvent->what == autoKey)
				{	*itemHit = 0;
					doHilite = 0;
				}
			}
		...
		{	switch(theTag)
			{	case ktsHostDomain:
					{	Rect	subRect;
						...
						subRect.right = theRect->right - 96;
						subRect.left = theRect->left;
						subRect.top = theRect->top;
						subRect.bottom = theRect->top + sText.nextLine;
		...
		{	char	index;
			...
			{	case kisPlayerNick:
					...
					{	Rect	subRect;
						...
						subRect.right = theRect->right;
						subRect.left = theRect->right - 96;
						subRect.top = theRect->top;
						subRect.bottom = theRect->top + sText.nextLine;
						TextBox(p.str, tagLen, &subRect, -1);
					}
					break;
			}
		}
	...
	{	case lInitMsg:
		...
		case lDrawMsg:
			...
			{	Rect	rectCopy = *rect;
				...
				DrawTrackerListItem(&rectCopy, data, len);
	...
	long	len)
{
	...
	{	if(curMessage != kStatReceiving &&
		...
		{	Str63		temp1, temp2;
			StringPtr	dest;
			short		i,j;
			Str31		totalString, indexString;
			
			NumToString(thisRecordNumber+1, indexString);
			NumToString(recordCount, totalString);
			 
			GetIndString(temp1, kTrackerStatusStrings, kStatReceiving);
			
			dest = temp2;
			for(i=1;i <= temp1[0];i++)
			{	unsigned char	theChar = temp1[i];
			
				if(theChar == '%')
				{	for(j=1; j<=indexString[0]; j++)
					{	*++dest = indexString[j];
					}
				}
				else
				if(theChar == '#')
				{	for(j=1; j<=totalString[0]; j++)
					{	*++dest = totalString[j];
					}
				}
				else
			...
			temp2[0] = dest - temp2;
			SetStatusDisplay(kStatReceiving, temp2);
		}
	}
		...
		{	case kTrackerPopItem:
			...
			case kQueryItem:
				{	SetCursor(&gWatchCursor);
					...
					WriteShortTag(ktsResponseCount, 500);
