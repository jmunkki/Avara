...
void	CBasicSound::SetLoopCount(
	short	count)
{
	loopCount[0] = loopCount[1] = count;
}

void	CBasicSound::SetRate(
	Fixed	theRate)
{
	//	Not applicable
}
...
void	CBasicSound::SetSoundLength(
	Fixed	seconds)
{
	long	numSamples;
...
void	CBasicSound::SetDirectVolumes(
	Fixed	vol0,
	Fixed	vol1)
{
...
{
	if(linkPtr)	linkPtr->refCount++;

	if(motionLink) itsHub->ReleaseLink(motionLink);
...
void	CBasicSound::SetControlLink(SoundLink *linkPtr)
{
	...
	if(controlLink) itsHub->ReleaseLink(controlLink);
...
void	CBasicSound::UseSamples(
	SampleHeaderHandle	theSample)
{
	if(theSample)
	{
...
void	CBasicSound::CalculatePosition(
	long		t)
{
	CSoundMixer	*m;
	SoundLink	*s;
	...
	for(i=0;i<3;i++)
	{
		relPos[i] =  s->loc[i]   - m->motion.loc[i] +
					(s->speed[i] - m->motion.speed[i]) * t;
		FSquareAccumulate(relPos[i], squareAcc);
	}

	if(0x007Fffff < (unsigned long)squareAcc[0])
		dSquare = 0x7FFFffff;
	else
		dSquare = *(Fixed *)(1+(char *)squareAcc);
...
void	CBasicSound::CalculateMotionVolume()
{
	...
	if(dSquare == 0) dSquare = m->distanceAdjust>>2;

	adjustedVolume = FMulDivNZ(masterVolume, m->distanceAdjust, dSquare);
	if(adjustedVolume > m->maxAdjustedVolume) adjustedVolume = m->maxAdjustedVolume;
	...
	{	distance = FSqroot(squareAcc);
		...
		if(m->strongStereo)
		{	//	"Hard" stereo effect for loudspeakers users.
		...
		{	//	"Soft" stereo effect for headphones users.
			//	Sound position	LEFT		MIDDLE		RIGHT
			...
			{	if(!volumes[0])	volumes[0] = 1;
				if(!volumes[1]) volumes[1] = 1;
			}
		}
	...
	if(volumes[0] > volumeMax) volumes[0] = volumeMax;
	if(volumes[1] > volumeMax) volumes[1] = volumeMax;

}

short	CBasicSound::CalcVolume(
	short	theChannel)
{
	if(controlLink)
...
void	CBasicSound::WriteFrame(
	short	theChannel,
	short	volumeAllowed)
{
	register	Sample			*s;
	register	WordSample		*d;
				SampleConvert	*converter;
	register	long			thisCount;
				long			remaining;
				long			baseCount = currentCount[0].i;
				long			loopCopy = loopCount[0];
...
{	
	...
	{	if(controlLink && controlLink->meta == metaSuspend)
		...
		if(loopCount)
		{	long	thisCount = itsMixer->soundBufferSize;
			long	remaining;
