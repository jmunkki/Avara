/*/
    Copyright й1994-1996, Juri Munkki
    All rights reserved.
...
void	CRateSound::UseSamplePtr(
	Sample	*samples,
	long	numSamples)
{
	inherited::UseSamplePtr(samples, numSamples);
	SetRate(FIX(1));
}

#define	MINIMUMRATE	(65536L>>(16-BASESOUNDBUFFERBITS))

void	CRateSound::SetRate(
	Fixed aRate)
{
	Fixed	adjustedRate;
	
	if(itsSamples)	adjustedRate = FMul((*itsSamples)->baseRate, aRate);
	else			adjustedRate = aRate;

	adjustedRate = FMul(adjustedRate, itsMixer->standardRate);

	if(adjustedRate < MINIMUMRATE) adjustedRate = MINIMUMRATE;
...
short	CRateSound::CalcVolume(
	short theChannel)
{
				short	theVol;
				long	endSample;
	unsigned	long	fracAdd;
	...
	{	chanDoneFlags[0] = chanDoneFlags[1] = false;
		...
		{	Fixed		startDistance;
			...
			if(m->stereo)	startDistance = distance;
			else			startDistance = FSqroot(squareAcc);
			...
			rateMultip = FIX(1) + (dopplerShift << (16-itsMixer->soundBufferBits));	//	еее

			if(rateMultip > FIX(4))			midRate = masterRate * 4;
			else if(rateMultip < FIX3(500))	midRate = masterRate >> 1;
			else							midRate = FMul(masterRate, rateMultip);

			if(m->stereo)
			{	Fixed	rateDiff;
			
				rateDiff = FMul(midRate, balance) >> (itsMixer->soundBufferBits-4);	//	еее
			...
			if(currentCount[0].i < -(chanRate[0] >> (16-itsMixer->soundBufferBits))	//	еее
				|| (loopCount[0] == 0 && currentCount[0].i >= sampleLen))
				volumes[0] = 0;
		
			if(itsMixer->stereo)
			{	if(currentCount[1].i < -(chanRate[1] >> (16-itsMixer->soundBufferBits))	//	еее
	...
	short	volumeAllowed)
{
	...
	long			didCount;
	long			maxOutput;
	long			loopCopy = loopCount[theChannel];
...
Boolean	CRateSound::EndFrame()
{
	...
	{	long			thisCount;
		...
		for(chan = 0; chan < chanCount; chan++)
		{
			...
			if(chanDoneFlags[chan] == false)
			{	rateCopy = chanRate[chan];
		
				fracAdd = ((unsigned short) rateCopy) << itsMixer->soundBufferBits;	//	еее
