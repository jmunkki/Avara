/*/
    ...
    File: CHuffProcessor.c
    Created: Thursday, August 31, 1995, 2:10
    Modified: Thursday, August 31, 1995, 20:57
/*/
...
long	CHuffProcessor::GetUncompressedLen(
	Ptr 	compressedData)
{
...
void	CHuffProcessor::Uncompress(
	Ptr	fromPtr,
	Ptr	destPtr)
{
	HuffDataHeader		*inHeader;
	...
	{
		freqCounts =  (unsigned long *)(sizeof(HuffDataHeader) + fromPtr);
...
long	CHuffProcessor::GetCompressedLen(
	Ptr	sourceData, long sourceLen)
{
	long			outSize;
	long			outputBits = 0;
	short			countDataSize;
	...
	Ptr	destPtr)
{
	...
	if(dataCount)
	{
		...
		if(dataCount > 65535)
		{	countTable = (unsigned long *)(sizeof(HuffDataHeader) + destPtr);
			for(i = 0;i < NUMSYMBOLS; i++)
			{	if(symbCounters[i])
