...
*/
void	CAbstractHuffPipe::SortNodes(
	HuffTreeNode	**table)
{
	short			h;
...
HuffTreeNode	*CAbstractHuffPipe::BuildTree()
{
	...
	/*	Place symbols into the first NUMSYMBOLS places
	**	in the tree array.
	*/
	for(i=0;i<NUMSYMBOLS;i++)
	...
	*/
	...
	/*
	**	Sort the pointers in the subTree array so that the symbol
	**	with the lowest count is first, etc.
	*/
	SortNodes(subTrees);	
...
/*
**	This abstract class doesn't pass any data forward. This method was
**	Used to test the tree creation code.
*/
OSErr	CAbstractHuffPipe::PipeData(
	Ptr dataPtr,
	long len)
{
	dataCount += len;
